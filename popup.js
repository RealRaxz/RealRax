...
<button id="pfYT1" class="pf-btn pf-red">
  ...
  <span>กดติดตาม คอมเมนต์ / Like & 𝗖𝗼𝗺𝗺𝗲𝗻𝘁</span>
</button>
...
if(window.lucide){
  ...
  yt1.onclick=()=>{
    if(done1) return;
    window.open("https://youtu.be/0qvz9UoyuvI","_blank");
    yt1Running=true;
  ...
  yt2.onclick=()=>{
    ...
    window.open("https://youtu.be/Zc5ezJXFnFs","_blank");
    yt2Running=true;
    lastTime=performance.now();
  ...
  yt3.onclick=()=>{
    ...
    window.open("https://youtu.be/vhA0mCmY6iI","_blank");
    yt3Running=true;
    lastTime=performance.now();
